...
const Color *COLOR_OFF = new Color{0, 0, 0, 0};
const Color *COLOR_ON = new Color{1, 1, 1, 1};
...
void DisplayBuffer::fill(const Color *color) {
...
void HOT DisplayBuffer::line(int x1, int y1, int x2, int y2, const Color *color) {
...
void HOT DisplayBuffer::horizontal_line(int x, int y, int width, const Color *color) {
  // Future: Could be made more efficient by manipulating buffer directly in certain rotations.
  for (int i = x; i < x + width; i++)
    this->draw_pixel_at(i, y, color);
}
void HOT DisplayBuffer::vertical_line(int x, int y, int height, const Color *color) {
  // Future: Could be made more efficient by manipulating buffer directly in certain rotations.
  for (int i = y; i < y + height; i++)
    this->draw_pixel_at(x, i, color);
}
void DisplayBuffer::rectangle(int x1, int y1, int width, int height, const Color *color) {
...
void DisplayBuffer::filled_rectangle(int x1, int y1, int width, int height, const Color *color) {
  // Future: Use vertical_line and horizontal_line methods depending on rotation to reduce memory accesses.
  for (int i = y1; i < y1 + height; i++) {
    this->horizontal_line(x1, i, width, color);
  }
}
void HOT DisplayBuffer::circle(int center_x, int center_xy, int radius, const Color *color) {
  int dx = -radius;
  int dy = 0;
  int err = 2 - 2 * radius;
  int e2;
...
void DisplayBuffer::filled_circle(int center_x, int center_y, int radius, const Color *color) {
...
void DisplayBuffer::print(int x, int y, Font *font, const Color *color, TextAlign align, const char *text) {
  int x_start, y_start;
  int width, height;
...
void DisplayBuffer::vprintf_(int x, int y, Font *font, const Color *color, TextAlign align, const char *format,
                             va_list arg) {
  char buffer[256];
...
void DisplayBuffer::image(int x, int y, Image *image, const Color *color_on, const Color *color_off) {
  switch (image->get_type()) {
    ...
    case IMAGE_TYPE_GRAYSCALE:
      for (int img_x = 0; img_x < image->get_width(); img_x++) {
        for (int img_y = 0; img_y < image->get_height(); img_y++) {
          Color pixel(image->get_grayscale_pixel(img_x, img_y));
          this->draw_pixel_at(x + img_x, y + img_y, &pixel);
        }
      }
      break;
    case IMAGE_TYPE_RGB24:
      for (int img_x = 0; img_x < image->get_width(); img_x++) {
        for (int img_y = 0; img_y < image->get_height(); img_y++) {
          Color pixel(image->get_color_pixel(img_x, img_y));
          this->draw_pixel_at(x + img_x, y + img_y, &pixel);
        }
      }
      break;
...
void DisplayBuffer::print(int x, int y, Font *font, const Color *color, const char *text) {
...
void DisplayBuffer::printf(int x, int y, Font *font, const Color *color, TextAlign align, const char *format, ...) {
  va_list arg;
  va_start(arg, format);
  this->vprintf_(x, y, font, color, align, format, arg);
  va_end(arg);
}
void DisplayBuffer::printf(int x, int y, Font *font, const Color *color, const char *format, ...) {
  va_list arg;
  va_start(arg, format);
...
#ifdef USE_TIME
void DisplayBuffer::strftime(int x, int y, Font *font, const Color *color, TextAlign align, const char *format,
                             time::ESPTime time) {
  char buffer[64];
...
void DisplayBuffer::strftime(int x, int y, Font *font, const Color *color, const char *format, time::ESPTime time) {
